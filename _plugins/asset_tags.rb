...
module Jekyll
  ...
  class LectureTag < Liquid::Tag
    ...
    def render(context)
      ...
      return 'Lecture not found.' unless lecture # in case the lecture isn't found
  ...
  # Add tags for lab releases, and link to the appropriate page.
  class LabReleaseTag < Liquid::Tag
    def initialize(tag_name, input, tokens)
      super
      # Split the input into a number and a title
      parts = input.strip.split(/\s+/, 2)
    ...
    def render(context)
      labs = context['site']['labs']
      lab = labs.find { |l| l.basename_without_ext == "lab#{@number}" }
      return 'Lab not found.' unless lab # in case the lecture isn't found

      lab_title = lab['title']
      lab_url = lab['external_url']
      # Directly grab the date string from the lecture data
      date_parts = lab['date'].to_s.split('-')
      year = date_parts[0].to_i
      month = date_parts[1].to_i
      day = date_parts[2].to_i
      if lab['time'].nil?
        ...
        hour = lab['time'] / 60 # lecture['time'] is expressed in minutes (i.e. 10:40 is 640)
        minute = lab['time'] % 60
      end
      # Construct the date manually
      release_time = Time.new(year, month, day, hour, minute)

      current_time = Time.now

      # Always render an anchor so client-side JS can toggle links in real time.
      # Attach a machine-readable ISO timestamp in `data-release`.
      release_iso = release_time.utc.iso8601
      # Use the lecture's URL (Jekyll document.url) and include the data-release attribute.
      "**Lab #{@number} released**{: .label .label-lab#{@number}-outline } <a href=\"#{lab_url}\" data-release=\"#{release_iso}\">#{lab_title}</a>"
    end
  end
  ...
  class ProjectReleaseTag < Liquid::Tag
    ...
    def text
      return "Lab #{@number}" if @number.to_i.to_s == @number
...
Liquid::Template.register_tag('lab', Jekyll::LabReleaseTag)
